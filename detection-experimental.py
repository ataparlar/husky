...
*PARAMETRELER
params sınıfı hemen hemen bütün parametrelerin depolandığı yerdir
...
-resizeRate: bu parammetrenin artırılması belli bir noktaya kadar algoritma kesinliğini arttırır lakin 
             daha fazla işlem gücü harcamasına neden olur
...
-showCandidate & showMarkers & showTresholded: bu üçü isminden de anlaşıacağı 
                                               üzere istediğiniz şeyleri kapatıp açabilirsiniz
...
class params:
    ...
    minMarkerDisRate = 1
    resizeRate = 4  # deneysel versiyondayız bu parametre şimdilik etkisiz
    cellMarginRate = 0.20
    markerSizeInBits = 5
    borderSizeInBits = 2
    errorCorrectionRate = 0.6
    correctedBits = 6
    configFilePath = 'logi-g922-config.json'
    undistortImg = False
    showCandidate = True
    showMarkers = True
    showTresholded = True
...
def validate_candidates(candidates, frame):
    ...
    for can in candidates:
        ...
        for valid in tags.valids:
            ...
            for i in range(4):
                error = np.count_nonzero(np.subtract(bits, validMarker))
                if error < params.errorCorrectionRate * params.correctedBits:
                    markers.append(can)
                    break
...
# DENEYSEL
def resize_pyr(inImg):
    return cv2.pyrUp(inImg)
...
def detect_candidates(grayImg):
    ...
    for c in cnts:
        # boyut kontrolü
        maxSize = int(max(grayImg.shape) * params.maxAreaRate)
        minSize = int(max(grayImg.shape) * params.minAreaRate)
...
# deneme amaçlı fonksiyon
def create_img_pyramid(inImg):
    outImg = np.zeros(inImg.shape)
    outImg = cv2.pyrUp(inImg, outImg)
    outImg = cv2.pyrUp(inImg, outImg)
    outImg = cv2.pyrUp(inImg, outImg)

    cv2.imshow("i", inImg)
    cv2.imshow('adas', outImg)
    return outImg
...
def main():
    ...
    if params.undistortImg is True:
        mtx, dist = load_camera_params(filename=params.configFilePath)
    ...
    while True:
        ...
        #frame = resize_pyr(frame)
        frame = cv2.GaussianBlur(frame, (3, 3), 0)

        # kamera kalibrasyon parametrelerine göre düzeltme uygulanır (istenirse)
        if params.undistortImg is True:
